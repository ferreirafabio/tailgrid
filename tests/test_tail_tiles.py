"""Tests for tail_tiles - Multi-tile tail viewer."""
...
class TestReadLastNLines:
    ...
    def test_read_exact_lines(self, tmp_path):
        from tail_tiles import read_last_n_lines
        f = tmp_path / "test.txt"
        f.write_text("line1\nline2\nline3\n")
...
class TestTailTile:
    ...
    def test_init(self, tmp_path):
        from tail_tiles import TailTile
        f = tmp_path / "test.txt"
        f.write_text("line1\n")
...
class TestLayouts:
    ...
    def test_layouts_exist(self):
        from tail_tiles import LAYOUTS
        assert '1' in LAYOUTS
        assert '2' in LAYOUTS
        assert '3' in LAYOUTS
        assert '4' in LAYOUTS
